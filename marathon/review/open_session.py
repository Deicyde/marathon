...
def build_open_prompt(
    ...
    *,
    ...
    workflow = (
        ...
        "hypothesis tightness, naming, downstream consumers. Note any "
        "stale snippets / shifted line refs. Drift is common after a "
        "reject — `marathon review reject N` launches a refine daemon "
        "that may run multiple Aristotle iterations against the code.\n"
        "3. **Walk the body's open questions one at a time.** Two "
        "sections need human input: *Informal Statement* (if still "
        "`⚠️ LLM-rendered`) and `### Verification questions`. Surface "
        "ONE item per turn; *wait*; on resolution **immediately** "
        "edit the body (`gh issue view N --json body --jq .body > "
        "/tmp/body.md`, edit, `gh issue edit N --body-file "
        "/tmp/body.md`) to remove the bullet or swap the `⚠️` marker "
        "for a `*✅ Verified*` one. Defer = leave, move on. Don't "
        "batch — one resolution = one body update.\n"
        "4. **Recommend a verdict** per Output format, once the "
        "open-questions section is drained. *Stop and wait* for the "
        "verdict signal.\n"
        "5. **Apply only on explicit human go-ahead.** When (and only "
        ...
        "6. **After a reject: poll the daemon, then re-review.** "
        "`marathon review reject` auto-launches a refine daemon "
        "iteration (10–30 min). Poll via `marathon review "
        "refine-status --chapter <N>` (chapter # in the role header); "
        "when the log shows `iteration complete` + a fresh commit "
        "hash, `git pull`, then re-run steps 1–4 against the new "
        "code (sync the body inline as part of step 3's walk). The "
        "loop is iterate-until-verify, not one-shot.\n"
    )

    rubric = (
        "## Decision rubric (three-axis critique)\n\n"
        "1. **Math correctness vs the textbook reference.** Statement "
        "matches what the source asserts? Use the issue's *Informal "
        "Statement* (human-approved per step 3) as the baseline.\n"
        "2. **Lean style / readability.** Idiomatic, variable-block "
        "discipline, `@[simp]`/`@[ext]` hooks, no `▸` ghosts.\n"
        "3. **Mathlib PR-readiness.** Naming, generality, no "
        "hypothesis bloat reinventing existing predicates.\n\n"
        "**Reject for**: wrong statement, dishonest return type, "
        "missing scaffolding, cross-chapter duplication, naming "
        "inconsistency, hypothesis-too-weak. **Not** for `sorry`s "
        "alone — the 🟡 marker advertises skeleton-with-sorries.\n"
    )
    ...
    output_format = (
        ...
        "with a clickable VS Code link "
        "`[file.lean:NN-MM](path/to/file.lean#LNN-LMM)` (markdown "
        "link, not bare backticks). No stylistic nitpicks unless "
        "load-bearing.\n"
        "* **Recommended action** (one line): exact marathon CLI command "
        "you'd run, with proposed `--comment` text.\n"
        "* **Then stop.** Don't run the CLI yet. **Stopping is the "
        "load-bearing step.**\n"
    )
    ...
    apply_section = (
        ...
        "**Never pass `--close`** unless told — sorry-free ≠ "
        "unimprovable, issues stay OPEN as tracking handles.\n"
        f"Reject: write `/tmp/issue{issue_num}-reject-notes.md`, then "
        f"`marathon review reject {issue_num} --notes "
        f"/tmp/issue{issue_num}-reject-notes.md --comment '…'`. "
        "After reject, proceed to workflow step 6 (poll + re-review).\n\n"
        "**Thread hygiene**: one one-line verdict comment per state "
        "transition (✅/❌/🟢/🟠); one substantive iteration comment "
        "per re-verify. No multi-paragraph verdicts, no @-tags.\n"
    )
