    ...
    **session continuation**: Marathon will dispatch the result via
    ...
    if pending_rejections_md:
        sections.append(
            ...
            "human reviewer rejected with explicit, file-and-declaration-"
            "level instructions for what they want changed. **The rejection "
            "is the EXCLUSIVE marquee move for this iteration.** The "
            "Aristotle prompt you draft must:\n\n"
            "1. Lead with the rejection's specific ask — name the same "
            "file(s), declaration(s), and structural change the human typed. "
            "Do not paraphrase the ask into something broader or narrower.\n"
            "2. NOT introduce other structural moves — no predicate "
            "unifications, no cross-file refactors, no `simp`-attribute "
            "audits, no docstring rewrites, no parallel-family "
            "consolidations, no hypothesis-tightening on adjacent decls, "
            "no \"while we're at it\" cleanup. Even if you notice "
            "structural items elsewhere in the target folder that look "
            "more urgent, defer them.\n"
            "3. NOT touch files outside those named in the rejection. If "
            "the rejection's change has unavoidable downstream consumer "
            "edits, include only those minimal consumer fixes.\n"
            "4. Surface other structural observations (if any) as a SHORT "
            "follow-up comment at the very end of your output, clearly "
            "labelled \"## Other items I noticed (not for this iteration)\". "
            "These observations are for the human's notes — Aristotle will "
            "not see them and must not act on them.\n\n"
            "Prior iterations have gone off-script by treating the rejection "
            "as a suggestion instead of a contract. Do not be the next. The "
            "human typed the reject notes as a specific instruction; honour "
            "them verbatim.\n\n"
